...
export async function getAllPostsCMS(): Promise<CMSPost[]> {
  ...
  // No DB sorts: some DBs don't have a Date property
...
export async function getPostCMS(slug: string): Promise<{ frontmatter: any; html: string } | null> {
  if (!hasNotion) {
  ...
  for (const b of blocks) {
    ...
    const o = (b as any)[t];
    if (!o) continue;
    if (t === "paragraph") htmlParts.push(`<p>${plain(o.rich_text)}</p>`);
    else if (t === "heading_1") htmlParts.push(`<h1>${plain(o.rich_text)}</h1>`);
    else if (t === "heading_2") htmlParts.push(`<h2>${plain(o.rich_text)}</h2>`);
    else if (t === "heading_3") htmlParts.push(`<h3>${plain(o.rich_text)}</h3>`);
    else if (t === "bulleted_list_item") htmlParts.push(`<ul><li>${plain(o.rich_text)}</li></ul>`);
    else if (t === "numbered_list_item") htmlParts.push(`<ol><li>${plain(o.rich_text)}</li></ol>`);
    else if (t === "quote") htmlParts.push(`<blockquote>${plain(o.rich_text)}</blockquote>`);
    else if (t === "divider") htmlParts.push(`<hr/>`);
    else if (t === "image") {
      const src = o.type === "external" ? o.external.url : o.file.url;
      const cap = plain(o.caption || []);
      htmlParts.push(`<figure><img src="${src}" alt="${cap || ""}"/><figcaption>${cap || ""}</figcaption></figure>`);
    } else {
      if (o.rich_text) htmlParts.push(`<p>${plain(o.rich_text)}</p>`);
    }
  }
